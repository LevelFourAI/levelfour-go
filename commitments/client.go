...
// One BUY- purchase: what it buys, the tranche it belongs to, and its latest purchase run with the steps, logs and trigger of its row. execution.status reads as it does for a renewal, plus returned when AWS refunded the plan after it was bought and cancelled when the plan was deleted while queued. execution.starts_at is when AWS starts a plan it queued. bought_outside names the plan of the same type and term bought outside LevelFour that stopped the latest run. Reads stored state only and never calls AWS. 404 for an id that is no purchase of this organization.
func (c *Client) GetPurchase(
	ctx context.Context,
	recommendationID string,
	opts ...option.RequestOption,
) (*levelfour.CommitmentPurchaseStatusResponse, error) {
	response, err := c.WithRawResponse.GetPurchase(
		ctx,
		recommendationID,
		opts...,
	)
